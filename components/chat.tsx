...
export function Chat({
  ...
  return (
    <div className="flex-1 flex flex-col py-4 gap-4 max-h-full max-w-[800px] mx-auto justify-between">
      <div
        id="chat-container"
        className="flex flex-col gap-2 overflow-y-auto max-h-full px-4 rounded-lg"
      >
        {messages.map((message: Message, index: number) => (
          <div
            className={`py-2 px-4 shadow-sm whitespace-pre-wrap ${message.role !== 'user' ? 'bg-white/5 border text-muted-foreground' : 'bg-white/20'} rounded-lg font-serif`}
            key={index}
          >
            {message.content.map((content, id) => {
              if (content.type === 'text') {
                return (
                  <p key={content.text} className="flex-1">
                    {content.text}
                  </p>
                )
              }
              ...
              if (content.type === 'image') {
                return (
                  <img
                    ...
                    src={content.image}
                    alt="artifact"
                    className="mr-2 inline-block w-[50px] h-[50px] object-contain border border-[#FFE7CC] rounded-lg bg-white mt-2"
                  />
                )
              }
            })}
      ...
      <div className="flex flex-col gap-4">
        <form onSubmit={handleSubmit} className="flex flex-row gap-2 items-center">
          <div className="relative">
            ...
            {files && (
              <div className="absolute top-[-3px] right-[-3px] bg-[#ff8800] text-white text-xs rounded-full h-4 w-4 flex items-center justify-center">
                {files.length}
              </div>
            )}
          </div>
